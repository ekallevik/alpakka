...
fn main() {
    ...
    let now = Utc::now();
    let trip_name = Text::new("Enter trip name...").prompt().unwrap();
    ...
    let selected_activities = MultiSelect::new("Select activities", keys).prompt().unwrap();
    ...
    println!("Creating trip: {trip_name}");
